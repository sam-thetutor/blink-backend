...
const { getCurrentNetwork, getBlockchainId } = require('../../config/stellar-config');
...
router.get('/', (req, res) => {
  ...
  const response = {
    ...
    description: "Send XLM instantly to any Stellar address. Choose from preset amounts or enter a custom amount. This Blink provides a complete interactive experience for XLM transfers.",
    links: {
      actions: [
        {
          type: "transaction",
  ...
  try {
    ...
    if (!/^G[A-Z0-9]{55}$/.test(recipient)) {
      return res.status(400).json({
        ...
        message: "Recipient must be a valid Stellar address (56 characters, starting with G)"
      });
    }
