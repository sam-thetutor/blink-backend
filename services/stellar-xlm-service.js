...
class StellarXLMService {
   ...
   */
  async createXLMTransferTransaction(sourceAccount, destinationAccount, amount, memo = '') {
    try {
      // Validate inputs
      if (!sourceAccount || !destinationAccount || !amount) {
        throw new Error('Missing required parameters: sourceAccount, destinationAccount, amount');
      }
      ...
      const sourceAccountObj = await this.horizonServer.loadAccount(sourceAccount);
  ...
  async getAccountInfo(accountId) {
    try {
      ...
      const response = await fetch(`${this.networkConfig.horizonUrl}/accounts/${accountId}`);
  ...
  async submitTransaction(signedTransactionXDR) {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Transaction failed: ${errorData.extras?.result_codes?.operations?.join(', ') || errorData.detail || 'Unknown error'}`);
      }
